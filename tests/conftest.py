"""Pytest configuration hooks for the phylogenetic rooting project."""
...
@pytest.fixture(
    params=[
        pytest.param(
            {
                "model_parameters": fixed_model_parameters([0.1, 0.2, 0.3]),
                "expected_model": "UNREST{0.1/0.2/0.3}",
            },
            id="fixed-parameters",
        ),
        pytest.param(
            {
                "model_parameters": parameter_distribution(
                    distribution_name="uniform",
                    draw_count=3,
                    parameters={"range": [0.5, 0.6]},
                ),
                "expected_model": "UNREST{0.547457/0.565747/0.566641}",
            },
            id="drawn-parameters",
        ),
    ]
)
def iqtree_model_parameter_case(request: pytest.FixtureRequest) -> dict[str, Any]:
    return request.param


@pytest.fixture
def iqtree_command_recorder(monkeypatch: pytest.MonkeyPatch) -> list[list[str]]:
    commands: list[list[str]] = []

    def fake_run(command, **kwargs):
        commands.append(list(command))
        if "--alisim" in command:
            prefix = Path(command[2])
            prefix.with_name(f"{prefix.name}.fasta").write_text(
                ">A\nAAAA\n>B\nCCCC\n", encoding="utf-8"
            )
